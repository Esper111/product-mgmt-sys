...
class MgmtSys:
    ...
    def fun(self):
            if self.aname.get()=="":
                ...
                self.list.append({"name":self.pname_e.get(),"pno":self.pno_e.get(),"q_e":self.q_e.get(),"price":self.price_e.get(),"dis":self.dis_e.get(),"bar":self.bar_e.get()})
    ...
    def fetch(self):
         ...
         for i in self.list:
                if self.rname_e.get()==i["name"]:
                    self.rno_e.set(i["pno"]) 
                    self.rbar_e.set(i["bar"])
                    self.rdis_e.set(i["dis"])
                    i["q_e"]=self.rq_e.get()
                print(i)
    ...
    def bill(self):
            for i in self.list:
                if self.pname_e1.get()==i["name"]:
                    self.pno_e1.set(i["pno"]) 
                    self.bar_e1.set(i["bar"])
                    self.dis_e1.set(i["dis"])
                    self.price_e1.set(i["price"])
                    h1=int(i["q_e"])
                    h2=int(self.q_e1.get())
                    if h1<h2:
                        messagebox.showerror("Quantity exceeded")
                        return                    
                    i["q_e"]=h1-h2
